...
class StatusBarUtil{
    ...
    fun setDefaultColor(activity: Activity) {
        var check = CheckOSUtil()
        if (check.isMIUI()) {
            setColor(activity, activity.resources.getColor(R.color._white), 0)
            setMiuiStatusBarDarkMode(activity, true)
        } else if (check.isFlyme()) {
            setColor(activity, activity.resources.getColor(R.color._white), 0)
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                setColor(activity, activity.resources.getColor(R.color._white), 50)
            }
        }
    }
}